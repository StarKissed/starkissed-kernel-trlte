...
static void boost_level(void)
{
	...
	stage = 1 << data->cmd_param[0];
	if (!(rmi4_data->tsp_booster->dvfs_stage & stage)) {
		snprintf(data->cmd_buff, sizeof(data->cmd_buff), "NG");
		data->cmd_state = CMD_STATUS_FAIL;
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: %d is not supported(%04x != %04x).\n",
				__func__, data->cmd_param[0],
				stage, rmi4_data->tsp_booster->dvfs_stage);

		goto boost_out;
	}

	rmi4_data->tsp_booster->dvfs_boost_mode = stage;

	snprintf(data->cmd_buff, sizeof(data->cmd_buff), "OK");
	data->cmd_state = CMD_STATUS_OK;

	if (rmi4_data->tsp_booster->dvfs_boost_mode == DVFS_STAGE_NONE) {
		retval = rmi4_data->tsp_booster->dvfs_off(rmi4_data->tsp_booster);
		if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev,
