...
static const char const *max77828_path_name[] = {
	[PATH_OPEN]	= "OPEN",
	[PATH_USB_AP]	= "USB-AP",
	[PATH_AUDIO]	= "AUDIO",
	[PATH_UART_AP]	= "UART-AP",
	[PATH_USB_AUX]	= "USB-AUX",
	[PATH_UART_AUX]	= "UART-AUX",
	NULL,
};
...
static void _detected(struct max77828_muic_info *info, u32 new_state);
...
__setup("pmic_77828=", get_if_pmic_inifo);
#endif
	...
	(struct max77828_muic_info *info, int type, int path)
{
	...
	if (type == 0) {
		...
		} else if (path == PATH_USB_AUX) {
			info->muic_data->usb_sel = PATH_USB_AUX;
...
static int max77828_muic_set_uart_sel_pass2
					  ...
					  const char *buf, size_t count)
{
	...
	if (!strncasecmp(buf, "PDA", 3)) {
		...
		max77828_muic_set_usb_path(info, PATH_USB_AUX);
	} else
					    ...
					    const char *buf, size_t count)
{
	...
	cntl1_val = (val << CTRL1_COMN1SW_SHIFT) | (val << CTRL1_COMP2SW_SHIFT) |
	    (0 << CTRL1_MICEN_SHIFT);
	cntl1_msk = CTRL1_COMN1SW_MASK | CTRL1_COMP2SW_MASK | CTRL1_MICEN_MASK;
...
void max77828_muic_set_chgdeten(int enable)
{
	struct max77828_muic_info *info = gInfo;
	struct i2c_client *client = info->muic;
...
/* WA code for force register setting */
static void max77828_set_dpdm_force_switch(struct max77828_muic_info *info) {
	struct i2c_client *client = info->muic;
	
	pr_info("%s: Force set DPDM Path to USB", __func__);

	/* Change D+/D- path using hidden register
	 * WA for change path to USB
	 * Addr : 0x08, ChgTypM(0x01) = '0'(Mask)
	 * Addr : 0xC2 / Value : 0xE7
	 * Addr : 0xC1 / Value : 0x00
	 * Addr : 0x31 / Value : 0x08
	 * Addr : 0xC3 / Value : 0xE7
	 * Addr : 0xC1 / Value : 0x00
	 * Addr : 0x0C / Value : 0x09 (SET USB PATH)
	 */
	
	disable_irq_nosync(info->irq_chgtype);
	max77828_write_reg(client, 0xC2, 0xE7);
	max77828_write_reg(client, 0xC1, 0x00);
	max77828_write_reg(client, 0x31, 0x08);
	max77828_write_reg(client, 0xC3, 0xE7);
	max77828_write_reg(client, 0xC1, 0x00);
}

static void max77828_unset_dpdm_force_switch(struct max77828_muic_info *info) {
	struct i2c_client *client = info->muic;
	
	pr_info("%s: Force unset DPDM Path to USB", __func__);

	/* Change D+/D- path using hidden register
	 * WA for change path to USB
	 * Addr : 0xC2 / Value : 0xE7
	 * Addr : 0xC1 / Value : 0x00
	 * Addr : 0x31 / Value : 0x00
	 * Addr : 0xC3 / Value : 0xE7
	 * Addr : 0xC1 / Value : 0x00
	 * Addr : 0x08, ChgTypM(0x01) = '1'(Unmask)
	 */

	max77828_write_reg(client, 0xC2, 0xE7);
	max77828_write_reg(client, 0xC1, 0x00);
	max77828_write_reg(client, 0x31, 0x00);
	max77828_write_reg(client, 0xC3, 0xE7);
	max77828_write_reg(client, 0xC1, 0x00);
	enable_irq(info->irq_chgtype);
}
#endif

#if defined(CONFIG_ADC_ONESHOT)
static int max77828_muic_set_adcmode(struct max77828_muic_info *info,
					int value)
{
	int ret;
	u8 val;
...
static bool is_muic_check_hv(struct max77828_muic_info *info)
{
	return (info->is_check_hv ? true : false);
}

static void max77828_muic_set_check_hv(struct max77828_muic_info *info, bool value)
{
...
static void max77828_muic_start_check_hv(struct max77828_muic_info *info)
{
	 ...
	 */
	max77828_write_reg(client, MAX77828_MUIC_REG_HVCTRL1, 0x11);

	/* Set VbusADCEn = 1 */
	max77828_update_reg(client, MAX77828_MUIC_REG_HVCTRL1,
			(1 << HVCTRL1_VBUSADCEN_SHIFT), HVCTRL1_VBUSADCEN_MASK);

	max77828_muic_set_check_hv(info, true);

	/* set INTMASK3 = 0x02 
	 * 0x02 = VDNMONM
	 */
	max77828_write_reg(client, MAX77828_MUIC_REG_INTMASK3, 0x02);

	return;
}

static void max77828_muic_set_hv(struct max77828_muic_info *info)
{
	struct i2c_client *client = info->muic;
	u8 value;
	u8 hvctrl1;
	...
	max77828_read_reg(info->muic, MAX77828_MUIC_REG_HVCTRL1, &hvctrl1);

	if ((hvctrl1 & 0x01) == 1) {
		max77828_read_reg(info->muic, MAX77828_MUIC_REG_STATUS3, &status3);
		if ((status3 & 0x10) != 0)
			return;
		else
	...
	max77828_muic_set_adcmode(info, ADC_ALWAYS);
#endif
	 ...
	 *  HVCTRL2 = 0x06
	 */
	max77828_write_reg(client, MAX77828_MUIC_REG_HVCTRL1, 0x20);
	max77828_write_reg(client, MAX77828_MUIC_REG_HVCTRL2, 0x06);

	/* Set INTMASK3 = 0x00 */
	max77828_write_reg(client, MAX77828_MUIC_REG_INTMASK3, 0x00);

	/* Set TX Data */
	dev_info(info->dev, "%s: Write TX DATA 0x%x\n", __func__, gInfo->tx_data);
	value = gInfo->tx_data;
	max77828_write_reg(client, MAX77828_MUIC_REG_HVTXBYTE, value);

	/* Set HVCTRL2 = 0x1B */
	max77828_write_reg(client, MAX77828_MUIC_REG_HVCTRL2, 0x1B);
#if defined(CONFIG_MUIC_SUPPORT_QC_CHARGER)
	/* Set INTMASK3 = 0x88 for MRxRdy / MPNack */
	ret = max77828_write_reg(client, MAX77828_MUIC_REG_INTMASK3, 0x88);
#else
	/* Set INTMASK3 = 0x80 for MRxRdy */
	ret = max77828_write_reg(client, MAX77828_MUIC_REG_INTMASK3, 0x80);
#endif
...
#if defined(CONFIG_MUIC_SUPPORT_QC_CHARGER)
static void max77828_muic_check_qc_hv(struct max77828_muic_info *info)
{
	u32 new_state=0;
	u8 status3;
	u8 hvctrl1;
	...
	ret = max77828_read_reg(info->muic, MAX77828_MUIC_REG_STATUS3, &status3);
	if(IS_ERR_VALUE(ret))
	{
	...
	max77828_write_reg(info->muic, MAX77828_MUIC_REG_HVCTRL2, 0x03);
	...
	max77828_write_reg(info->muic, MAX77828_MUIC_REG_HVCTRL1, hvctrl1);
	...
	if(((status3 & 0x0F) >= MVBUS_8V_9V) && ((status3 & 0x0F) <= MVBUS_9V_10V))
	{
		pr_info("%s: VbADC is right QC 2.0 value.\n", __func__);
		new_state = BIT(EXTCON_HV_TA);
		gInfo->cable_name = EXTCON_HV_TA;
	} else if(((status3 & 0x0F) >= MVBUS_4V_5V) && ((status3 & 0x0F) <= MVBUS_5V_6V)) {
		pr_info("%s: VbADC is in normal voltage value.(adc=0x%x)\n", __func__, (status3&0x0F));
		max77828_write_reg(info->muic, MAX77828_MUIC_REG_HVCTRL1, 0x33);
		new_state = BIT(EXTCON_TA);
		gInfo->cable_name = EXTCON_TA;
	} else {
		pr_info("%s: VbADC is out of QC 2.0 range value.(%x)\n", __func__, (status3&0x0F));
#if defined(CONFIG_ADC_ONESHOT)
		max77828_muic_set_adcmode(info, ADC_ONESHOT);
#endif
		return;
	}
...
static void max77828_muic_check_handshaking(struct max77828_muic_info *info)
{
	struct i2c_client *client = info->muic;
	u32 new_state=0;
	u8 hvtxbyte;
	...
	if (info->cable_name == EXTCON_HV_TA_1A){
		...
		max77828_write_reg(client, MAX77828_MUIC_REG_INTMASK3, 0x00);
		max77828_write_reg(client, MAX77828_MUIC_REG_HVCTRL2, 0x03);
		new_state = BIT(EXTCON_HV_TA_1A);
		gInfo->cable_name = EXTCON_HV_TA_1A;
		goto __found_cable;
	}

	max77828_read_reg(client, MAX77828_MUIC_REG_HVTXBYTE, &hvtxbyte);

	ret = max77828_bulk_read(client, MAX77828_MUIC_REG_HVRXBYTE1, 16, hvrxbyte);
	if(ret) {
	...
	if(hvrxbyte[0] != hvtxbyte)
	{
		...
		if (selecthvtxbyte != 0)
			max77828_write_reg(client, MAX77828_MUIC_REG_HVTXBYTE, selecthvtxbyte);
		else {
			new_state =  BIT(EXTCON_TA);
			gInfo->cable_name = EXTCON_TA;
			goto __found_cable;
	 ...
	 */
	max77828_write_reg(client, MAX77828_MUIC_REG_HVCTRL2, 0x5B);

	msleep_interruptible(400);

	/* Read VbADC in STATUS3 reg */
	ret = max77828_read_reg(client, MAX77828_MUIC_REG_STATUS3, &value);
	if(IS_ERR_VALUE(ret))
	{
		dev_err(info->dev, "%s: cannot read STATUS3: %d\n", __func__, ret);
		return;
	...
	case MVBUS_9V_10V:
		...
		max77828_write_reg(client, MAX77828_MUIC_REG_INTMASK3, 0x00);
		max77828_write_reg(client, MAX77828_MUIC_REG_HVCTRL2, 0x03);
		max77828_muic_set_check_hv(info, false);
		break;
	case MVBUS_4V_5V:
	case MVBUS_5V_6V:
		pr_info("%s: VbADC is in normal voltage value.(adc=0x%x)\n", __func__, vbadc);
...
static u8 max77828_muic_delay_for_vdnmon(struct max77828_muic_info *info)
{
	int ret;
	u8 status3;
	u8 vdnmon;
			...
			struct device_attribute *attr, char *buf)
{
	...
	ret = max77828_read_reg(info->muic, MAX77828_MUIC_REG_CTRL4, &val);
	if (ret) {
		dev_err(info->dev, "%s: fail to read muic reg\n", __func__);
		return sprintf(buf, "UNKNOWN\n");
	}
...
static ssize_t max77828_muic_set_afc_disable(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	struct max77828_muic_info *info = dev_get_drvdata(dev);
...
static ssize_t max77828_muic_show_afc_disable(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct max77828_muic_info *info = dev_get_drvdata(dev);

	dev_info(info->dev, "func: AFC is %s %s\n", __func__,
...
static ssize_t max77828_muic_set_uart_en(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	struct max77828_muic_info *info = dev_get_drvdata(dev);
...
static ssize_t max77828_muic_show_uart_en(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct max77828_muic_info *info = dev_get_drvdata(dev);
					  ...
					  const char *buf, size_t count)
{
	...
	if (!strncasecmp(buf, "AP", 2)) {
		int ret = max77828_muic_set_uart_sel_pass2
			(info, PATH_UART_AP);
		if (ret >= 0){
			info->muic_data->uart_sel = PATH_UART_AP;
...
static DEVICE_ATTR(afc_disable, 0664, max77828_muic_show_afc_disable,
		max77828_muic_set_afc_disable);
#if defined(CONFIG_MUIC_SUPPORT_RUSTPROOF)
static DEVICE_ATTR(uart_en, 0664, max77828_muic_show_uart_en,
		max77828_muic_set_uart_en);
#endif
...
static DEVICE_ATTR(attached_dev, S_IRUGO, max77828_muic_show_device, NULL);
...
static void max77828_muic_set_disable_chgdetrun(struct max77828_muic_info *info)
{
	pr_info("%s:Force disable for CHG_DET_RUN\n", __func__);
	max77828_update_reg(info->muic, MAX77828_MUIC_REG_CDETCTRL1,
			0x0 << CDETCTRL1_CHGDETEN_SHIFT, CDETCTRL1_CHGDETEN_MASK);
}

static void max77828_muic_set_enable_chgdetrun(struct max77828_muic_info *info)
{
	pr_info("%s:Force disable for CHG_DET_RUN\n", __func__);
	max77828_update_reg(info->muic, MAX77828_MUIC_REG_CDETCTRL1,
			0x1 << CDETCTRL1_CHGDETEN_SHIFT, CDETCTRL1_CHGDETEN_MASK);
}


static void max77828_muic_restore_wa_setting(struct max77828_muic_info *info)
{
	pr_info("%s:Forse disable WA codes.\n", __func__);
	max77828_muic_set_enable_chgdetrun(info);
#if defined(CONFIG_MUIC_SUPPORT_SMART_DOCK)
	max77828_unset_dpdm_force_switch(info);
#endif
}
...
static int max77828_muic_set_path(struct max77828_muic_info *info, int path)
{
	...
	ctrl1_val = (val << CTRL1_COMN1SW_SHIFT) | (val << CTRL1_COMP2SW_SHIFT);
	ctrl1_msk = CTRL1_COMN1SW_MASK | CTRL1_COMP2SW_MASK;

	if (path == PATH_AUDIO) {
		ctrl1_val |= 0 << CTRL1_MICEN_SHIFT;
		ctrl1_msk |= CTRL1_MICEN_MASK;
	}

	max77828_update_reg(info->muic,
				MAX77828_MUIC_REG_CTRL1, ctrl1_val, ctrl1_msk);

	ctrl2_val = CTRL2_CPEn1_LOWPWD0;
	ctrl2_msk = CTRL2_CPEn_MASK | CTRL2_LOWPWD_MASK;

	max77828_update_reg(info->muic,
			MAX77828_MUIC_REG_CTRL2, ctrl2_val, ctrl2_msk);

	return 0;
}

#if defined(CONFIG_MUIC_SUPPORT_HMT_DETECTION)
#define MAX77828_PATH_FIX_USB_MASK \
...
#endif
#define MAX77828_PATH_USB_MASK \
	(BIT(EXTCON_USB) | BIT(EXTCON_JIG_USBOFF) | BIT(EXTCON_JIG_USBON) | BIT(EXTCON_CHARGE_DOWNSTREAM))
#define MAX77828_PATH_AUDIO_MASK BIT(EXTCON_DESKDOCK)
#define MAX77828_PATH_UART_MASK	\
	(BIT(EXTCON_JIG_UARTOFF) | BIT(EXTCON_JIG_UARTON) | BIT(EXTCON_USB_HOST_5V))

static int set_muic_path(struct max77828_muic_info *info)
{
	...
	dev_info(info->dev, "%s: current state=0x%x, path=%s\n",
			__func__, state, max77828_path_name[info->path]);

	if (state & MAX77828_PATH_FIX_USB_MASK)
		path = PATH_USB_AP;
	else if (state & MAX77828_PATH_USB_MASK) {
		/* set path to usb following usb_sel */
		if (info->muic_data->usb_sel == PATH_USB_AUX)
			path = PATH_USB_AUX;
		else
			path = PATH_USB_AP;
	} else if (state & MAX77828_PATH_UART_MASK) {
		/* set path to uart following uart_sel */
		if (info->muic_data->uart_sel == PATH_UART_AUX)
			path = PATH_UART_AUX;
		else
			path = PATH_UART_AP;
	} else if (state & MAX77828_PATH_AUDIO_MASK)
		path = PATH_AUDIO;
	else {
	...
	ret = max77828_muic_set_path(info, path);
	if (ret < 0) {
		dev_err(info->dev, "fail to set path(%s->%s),stat=%x,ret=%d\n",
				max77828_path_name[info->path],
				max77828_path_name[path], state, ret);
		return ret;
	}

	dev_info(info->dev, "%s: path: %s -> %s\n", __func__,
			max77828_path_name[info->path],
			max77828_path_name[path]);
	info->path = path;
				       ...
				       u8 status1, u8 status2, u8 status3, int irq)
{
	 ...
	 */
#if 0
	if (info->max77828->pmic_rev > MAX77828_REV_PASS1) {
	...
	switch (adc) {
	case ADC_GND:
		max77828_muic_set_disable_chgdetrun(info);
	...
	case ADC_SMARTDOCK:
		...
		if (!max77828_muic_set_adcmode(info, ADC_ALWAYS))
			msleep(50);
#endif
		...
		if (chgtyp == CHGTYP_DEDICATED_CHGR) {
			 ...
			 * - D+/D- path is not changed when MMD attach.
			 *   forse register setting for path chainging
			 */
			max77828_set_dpdm_force_switch(info);
			...
			case CHGTYP_RFU:
				...
				if (!info->afc_disable) {
					if ((!is_muic_check_hv(info) && chgtyp == CHGTYP_DEDICATED_CHGR)
						 ...
						 */
						if(irq == -1)
							max77828_muic_delay_for_vdnmon(info);
						if(poweroff_charging || irq == -1) {
							max77828_muic_start_check_hv(info);
							max77828_muic_set_hv(info);
						}
					}
				}
				new_state = BIT(EXTCON_TA);
...
static int max77828_muic_handle_detach(struct max77828_muic_info *info)
{
	u8 ctrl2_val;
...
#if defined(CONFIG_MUIC_SUPPORT_SMART_DOCK)
	/* WA code for MMD path change
	 * - unset force register setting
	 */
	...
	max77828_write_reg(info->muic, MAX77828_MUIC_REG_CTRL1,
					MAX77828_MUIC_CTRL1_BIN_0_000);

	/* Enable Factory Accessory Detection State Machine */

	max77828_update_reg(info->muic,
				MAX77828_MUIC_REG_CTRL2, CTRL2_CPEn0_LOWPWD1,
				CTRL2_CPEn_MASK | CTRL2_LOWPWD_MASK);

	max77828_read_reg(info->muic, MAX77828_MUIC_REG_CTRL2,
				&ctrl2_val);
	dev_info(info->dev, "%s: CNTL2(0x%02x)\n", __func__, ctrl2_val);
...
static void max77828_muic_detect_dev(struct max77828_muic_info *info, int irq)
{
	...
	ret = max77828_bulk_read(client, MAX77828_MUIC_REG_STATUS1, 3, status);
	...
	if (intr == INT_ATTACH) {
		...
		max77828_muic_handle_attach(info, status[0], status[1], status[2], irq);
	} else if (intr == INT_DETACH) {
		dev_info(info->dev, "%s: DETACHED\n", __func__);
		max77828_muic_handle_detach(info);
	} else {
...
static irqreturn_t max77828_muic_mrxrdy_irq(int irq, void *data)
{
	struct max77828_muic_info *info = data;
	dev_info(info->dev, "%s: irq:%d\n", __func__, irq);

	mutex_lock(&info->mutex);
	max77828_muic_check_handshaking(info);
	mutex_unlock(&info->mutex);

	return IRQ_HANDLED;
}

static irqreturn_t max77828_muic_vdnmon_irq(int irq, void *data)
{
	struct max77828_muic_info *info = data;
	dev_info(info->dev, "%s: irq:%d\n", __func__, irq);
...
#if defined(CONFIG_MUIC_SUPPORT_QC_CHARGER)
static irqreturn_t max77828_muic_mpnack_irq(int irq, void *data)
{
	struct max77828_muic_info *info = data;
	dev_info(info->dev, "%s: irq:%d\n", __func__, irq);
	
	mutex_lock(&info->mutex);
	max77828_muic_check_qc_hv(info);
	mutex_unlock(&info->mutex);
...
do {									\
	ret = request_threaded_irq(_irq, NULL, max77828_muic_mpnack_irq,\
				    0, _name, info);			\
	if (ret < 0) {							\
...
do {									\
	ret = request_threaded_irq(_irq, NULL, max77828_muic_mrxrdy_irq,\
				    0, _name, info);			\
	if (ret < 0) {							\
		dev_err(info->dev, "Failed to request IRQ #%d: %d\n",	\
			_irq, ret);					\
...
do {									\
	ret = request_threaded_irq(_irq, NULL, max77828_muic_vdnmon_irq,\
				    0, _name, info);			\
	if (ret < 0) {							\
		dev_err(info->dev, "Failed to request IRQ #%d: %d\n",	\
			_irq, ret);					\
...
static int max77828_muic_irq_init(struct max77828_muic_info *info)
{
	...
	max77828_write_reg(info->muic, MAX77828_MUIC_REG_INTMASK3, 0x02);

	for (count = 1; count < 9 ; count ++) {
		max77828_read_reg(info->muic, count, &val);
...
static int max77828_muic_probe(struct platform_device *pdev)
{
	...
	info->irq_vdnmon = max77828->irq_base + MAX77828_MUIC_IRQ_INT3_VDNMON;
	info->irq_mrxrdy = max77828->irq_base + MAX77828_MUIC_IRQ_INT3_MRXRDY;
#if defined(CONFIG_MUIC_SUPPORT_QC_CHARGER)
	info->irq_mpnack = max77828->irq_base + MAX77828_MUIC_IRQ_INT3_MPNACK;
#endif
	...
	if ((switch_sel & MAX77828_SWITCH_SEL_2nd_BIT_UART) == 0x1 << 1)
		info->muic_data->uart_sel = PATH_UART_AP;
	else
		info->muic_data->uart_sel = PATH_UART_AUX;
	...
	/*  Get ADC Mode value */
	max77828_read_reg(info->max77828->muic,
		MAX77828_MUIC_REG_CTRL4, &info->adc_mode);
	...
	if ( (switch_sel&0x08) == 0x00 ) {
		/* MAKE PATH OPEN */
		max77828_muic_set_path(info, PATH_OPEN);
...
static int max77828_suspend(struct device *dev)
{
	...
	val = (0 << CTRL2_LOWPWD_SHIFT);

	ret = max77828_update_reg(info->muic, MAX77828_MUIC_REG_CTRL2, val,
			CTRL2_LOWPWD_MASK);
	if (ret < 0) {
		dev_err(info->dev, "%s: fail to update reg\n", __func__);
		return -1;
...
static const struct dev_pm_ops max77828_dev_pm_ops = {
