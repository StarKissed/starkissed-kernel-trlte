		...
		if (val->intval == POWER_SUPPLY_TYPE_POWER_SHARING) {
			...
			if (value.intval) {
				max77804k_update_reg(charger->max77804k->i2c, MAX77804K_CHG_REG_CHG_CNFG_00,
					MAX77804K_CHG_CNFG_00_OTG_CTRL, MAX77804K_CHG_CNFG_00_OTG_CTRL);
				pr_info("%s: ps enable\n", __func__);
			} else {
				max77804k_update_reg(charger->max77804k->i2c, MAX77804K_CHG_REG_CHG_CNFG_00,
					0, MAX77804K_CHG_CNFG_00_OTG_CTRL);
...
static int max77804k_otg_get_property(struct power_supply *psy,
				enum power_supply_property psp,
				union power_supply_propval *val)
{
	return 0;
...
static int max77804k_otg_set_property(struct power_supply *psy,
				enum power_supply_property psp,
				const union power_supply_propval *val)
{
	struct max77804k_charger_data *charger =
		container_of(psy, struct max77804k_charger_data, psy_otg);
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (val->intval) {
			...
			max77804k_update_reg(charger->max77804k->i2c, MAX77804K_CHG_REG_CHG_INT_MASK,
				MAX77804K_CHG_IM | MAX77804K_CHGIN_IM,
				MAX77804K_CHG_IM | MAX77804K_CHGIN_IM | MAX77804K_BYP_IM);

			/* disable charger detection */
			max77804k_update_reg(charger->max77804k->muic, MAX77804K_MUIC_REG_CDETCTRL1,
				0, MAX77804K_CHGDETEN_MASK);
			...
			max77804k_update_reg(charger->max77804k->muic, MAX77804K_MUIC_REG_CDETCTRL1,
				MAX77804K_CHGDETEN_MASK, MAX77804K_CHGDETEN_MASK);
...
static void max77804k_charger_initialize(struct max77804k_charger_data *charger)
{
	 ...
	 */
	max77804k_update_reg(charger->max77804k->i2c,
		MAX77804K_CHG_REG_CHG_CNFG_02, 1 << 7, 1 << 7);
...
static int max77804k_charger_probe(struct platform_device *pdev)
{
	...
	charger->psy_chg.name           = "max77804k-charger";
