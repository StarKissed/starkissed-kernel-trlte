...
{
	printk(KERN_ERR"\nRKP2 Depth:Proc = #%d:%s# ppid:pname= #%d:%s# \nRKP uid = %d gid = %d euid = %d  egid = %d \n",current->cred->exec_depth,current->comm,(unsigned int)current->real_parent->pid,current->real_parent->comm,current->cred->uid,current->cred->gid,current->cred->euid,current->cred->egid);
	printk(KERN_ERR"\n RKP3 cred = %p bp_task = %p, current = %p , current->mm =%p\n",current->cred,current->cred->bp_task,current,current->mm);
	if(current->mm)
		printk(KERN_ERR"\n RKP pgd = %lx bp_pgd %lx\n",(unsigned long)current->mm->pgd,(unsigned long)current->cred->bp_pgd);
...
//#ifndef CONFIG_TIMA_RKP_COHERENT_TT
		v7_flush_kern_dcache_area(&cred->security, 4);
//#endif
		if ((unsigned long) cred->security != 7) {
			printk(KERN_ERR"RKP CRED SECURITY %s -> %p sec %p SHOULD BE 7\n", 
					__func__, cred, cred->security);
			panic("RKP MEM WRITE DID NOT WORK");
		}
	} else
#endif /*CONFIG_TIMA_RKP_RO_CRED*/
